					...
					if (i < argc && atom_gettype (argv + i) == A_FLOAT)
						...
						{	
							if (x->oscarTransfer == TRANSFER_LINEAR || x->oscarTransfer == TRANSFER_TANH)
								{
									x->vectorIn[i] = 
										(double)(((CLAMP (atom_getlong (argv + i), 0, 127) * 2.) / 127.) - 1.);
								}
							else
								{
									x->vectorIn[i] = 
										(double)(CLAMP (atom_getlong (argv + i), 0, 127) / 127.);
								}
							...
							if (x->papaTransfer == TRANSFER_LINEAR || x->papaTransfer == TRANSFER_TANH)
								{
									...
									atom_setlong (result + j, CLAMP (n, 0, 127));
								}
							else
								{
					...
					if (i < argc && atom_gettype (argv + i) == A_FLOAT)
						...
						{	
							if (x->oscarTransfer == TRANSFER_LINEAR || x->oscarTransfer == TRANSFER_TANH)
								...
								{
									x->vectorIn[i] = (double)(CLAMP (atom_getlong (argv + i), 0, 127) / 127.);
								}
						}
					else
						{
