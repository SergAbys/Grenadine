...
bool tralala_moveSelectedNotes (t_tralala *x, long deltaPosition, long deltaPitch)
{
    ...
    for (i = 0; i < count; i += PIZ_DATA_NOTE_SIZE) {
        ...
        position = CLAMP (position, 0, PIZ_DEFAULT_TIMELINE_SIZE - duration);
...
bool tralala_changeSelectedNotesDuration (t_tralala *x, long deltaPosition)
{
    ...
    for (i = 0; i < count; i += PIZ_DATA_NOTE_SIZE) {
        ...
        maximum  = MIN (PIZ_SEQUENCE_MAXIMUM_DURATION, (PIZ_DEFAULT_TIMELINE_SIZE - position));
...
bool tralala_pasteFromClipboard (t_tralala *x) 
{
    ...
    long w = (PIZ_DEFAULT_TIMELINE_SIZE / 2);
